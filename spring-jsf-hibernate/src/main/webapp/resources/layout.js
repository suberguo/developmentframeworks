...
$(document).ready(function() {
	...
	LAYOUT.menu.perfectScrollbar({
		wheelSpeed: 40,
		suppressScrollX:true
	});
    	
	// menu mouseenter & mouseleave actions start ----------------------------------
	LAYOUT.menu.on("mouseenter", function() {
        LAYOUT.highlightMenu();
	})
	.on("mouseleave", function() {
        LAYOUT.unhighlightMenu();
	});
    
    LAYOUT.mobileMenuBtn.on('click', function(e) {
       LAYOUT.menu.toggleClass('active-menu'); 
       e.preventDefault();
    });
    ...
    $('#mobilemenu').on('change', function(e) {
     ...
     // Search ---------------------------------------
    LAYOUT.searchInput.on('keyup', function(e) {
        LAYOUT.hideSubMenus();
